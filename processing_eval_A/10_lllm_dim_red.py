...
nlt_embs = df['llm_embedding'].apply(lambda x: np.fromstring(x, sep=','))
...
df_2d.to_csv("ontouml_embeddings_llm_2d.csv", index=False)
...
plt.tight_layout()
plt.savefig("ontouml_embeddings_llm_2d.png", dpi=300)
plt.close()
...
df_3d.to_csv("ontouml_embeddings_llm_3d.csv", index=False)
...
ax.set_title("OntoUML Embeddings: LLM to CMT (3D)", fontsize=14)
plt.tight_layout()
plt.savefig("ontouml_embeddings_llm_3d.png", dpi=300)
plt.close()
