...
base_path = 'datasets/ontouml-models-master/models'
extracted_data = []
    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            ...
            if key == 'name' and isinstance(value, str) and value.strip():
                ...
                # Try to fetch 'type' and 'stereotype' from the same object
                for attr in ['type', 'stereotype']:
...
for folder in os.listdir(base_path):
    ontology_path = os.path.join(base_path, folder, 'ontology.json')
...
# Save output
with open('extracted_ontouml.json', 'w', encoding='utf-8') as out_file:
    json.dump(extracted_data, out_file, indent=2, ensure_ascii=False)

print(f"✅ Extracted {len(extracted_data)} name entries with paths and metadata to extracted_ontouml.json")
