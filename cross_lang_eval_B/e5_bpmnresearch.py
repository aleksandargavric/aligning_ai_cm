...
# -----------------------------
with open('extracted_bpmnresearch.json', 'r') as f:
    data = json.load(f)
...
for model, items in groups.items():
    ...
    for item in items:
        ...
        if not term:
            continue
        cmt_str = f"bpmn term '{term}'"
        term_type = item.get("id")
        stereotype = ", ".join(item.get("path"))
...
df.to_csv('model_embeddings_bpmnresearch.csv', index=False)
