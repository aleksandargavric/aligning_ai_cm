...
# -----------------------------
with open('extracted_eamodelset.json', 'r') as f:
    data = json.load(f)
...
for model, items in groups.items():
    ...
    for item in items:
        ...
        if not term:
            continue
        cmt_str = f"Archimate term '{term}'"
        term_type = item.get("type")
        stereotype = item.get("layer")
        if term_type and str(term_type).strip():
            cmt_str += f" is of type '{term_type}'"
        if stereotype and str(stereotype).strip():
            # If a type was added, then add stereotype with "and"
            if "is of type" in cmt_str:
                cmt_str += f" and it's layer is '{stereotype}'"
            else:
                cmt_str += f" with layer name '{stereotype}'"
...
df.to_csv('model_embeddings_eamodelset.csv', index=False)
