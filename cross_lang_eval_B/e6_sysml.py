...
# -----------------------------
with open('extracted_sysphs.json', 'r') as f:
    data = json.load(f)
...
for model, items in groups.items():
    ...
    for item in items:
        ...
        if not term:
            continue
        cmt_str = f"sysml term '{term}'"
        term_type = item.get("id")
        stereotype = ", ".join(dict.fromkeys(item for item in item.get("path") if item != "XMI"))
        if term_type and str(term_type).strip():
            cmt_str += f" is of id '{term_type}'"
        if stereotype and str(stereotype).strip():
            # If a type was added, then add stereotype with "and"
            if "is of id" in cmt_str:
                cmt_str += f" and it's attrubutes are '{stereotype}'"
            else:
                cmt_str += f" with attributes '{stereotype}'"
...
df.to_csv('model_embeddings_sysphs.csv', index=False)
...
reductions = {}
for method, func in zip(["PCA", "t-SNE"], [reduce_pca, reduce_tsne]):
