...
base_path = 'datasets/eamodelset/dataset/processed-models'
extracted_data = []
...
for folder in os.listdir(base_path):
    ontology_path = os.path.join(base_path, folder, 'model.json')
...
# Save output
with open('extracted_eamodelset.json', 'w', encoding='utf-8') as out_file:
    json.dump(extracted_data, out_file, indent=2, ensure_ascii=False)

print(f"✅ Extracted {len(extracted_data)} name entries with paths and metadata to extracted_eamodelset.json")
