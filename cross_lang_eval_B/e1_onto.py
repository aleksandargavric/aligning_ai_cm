...
# -----------------------------
with open('extracted_ontouml.json', 'r') as f:
    data = json.load(f)
...
for model, items in groups.items():
    ...
    for item in items:
        ...
        if not term:
            continue
        cmt_str = f"ontouml extracted term '{term}'"
        ...
        if stereotype and str(stereotype).strip():
            ...
            if "is of type" in cmt_str:
                cmt_str += f" and it's stereotype is '{stereotype}'"
            else:
                cmt_str += f" with stereotype '{stereotype}'"
...
df.to_csv('model_embeddings.csv', index=False)
